...
namespace PicaVoxel
{
    ...
    0xff880000, 0xff770000, 0xff550000, 0xff440000, 0xff220000, 0xff110000, 0xffeeeeee, 0xffdddddd, 0xffbbbbbb, 0xffaaaaaa, 0xff888888, 0xff777777, 0xff555555, 0xff444444, 0xff222222, 0xff111111
		...
		public static void FromMagica(BinaryReader stream, GameObject root, float voxelSize, bool centerPivot, float lodScale)
        {
            ...
            if (magic == "VOX ")
            {
				int srcSizex = 0, srcSizey = 0, srcSizez = 0;
                ...
                while (stream.BaseStream.Position < stream.BaseStream.Length)
                {
                    ...
                    if (chunkName == "SIZE")
                    {
						srcSizex = stream.ReadInt32();
						srcSizez = stream.ReadInt32();
						srcSizey = stream.ReadInt32();

						// Take LOD into account
                        sizex = (int)Mathf.Max(1, srcSizex * lodScale);
                        sizey = (int)Mathf.Max(1, srcSizey * lodScale);
                        sizez = (int)Mathf.Max(1, srcSizez * lodScale);
                    ...
                    else if (chunkName == "RGBA")
                    {
                        ...
                        for (int i = 0; i < 256; i++)
                        {
                            ...
                            // Hack - Use unique alpha values to identify first 16 palette colours
                            float val = Mathf.Clamp01((float)i / 15.0f);
                            a = (byte)(val * 255);
                ...
                if (root != null)
                {
                    ...
                    foreach (MagicaVoxelData v in voxelData)
                    {
                        try
                        {
                            ...
                            x = (int)Mathf.Min(x * lodScale, sizex - 1);
                            y = (int)Mathf.Min(y * lodScale, sizey - 1);
                            z = (int)Mathf.Min(z * lodScale, sizez - 1);

                            Voxel voxel = voxelVolume.Frames[0].Voxels[x + sizex * (y + sizey * z)];

                            if (voxel.State != VoxelState.Active)
                            {
                                voxel.State = VoxelState.Active;
                                voxel.Color = colors[v.color-1];
                                voxel.Value = 128;
                            }

                            voxelVolume.Frames[0].Voxels[x + sizex * (y + sizey * z)] = voxel;
                        }
                        catch (Exception e)
                        {
							Debug.LogFormat("srcSize: {0},{1},{2}", srcSizex, srcSizey, srcSizez);
							Debug.LogFormat("Size: {0},{1},{2}", sizex, sizey, sizez);
							Debug.Log("MagicaVoxel Import Error at " + v.x + " " + v.y + " " + v.z + " : " + e.ToString());
                        }
                    }

                    voxelVolume.transform.localScale = new Vector3(1.0f / lodScale, 1.0f / lodScale, 1.0f / lodScale);
		...
		public static GameObject MagicaVoxelImport(string fn, string volumeName, float voxelSize, bool centerPivot, MeshingMode meshingMode, float lodScale)
        {
            ...
            Volume volume = newObject.GetComponent<Volume>();
            volume.Material = EditorUtility.PicaVoxelDiffuseMaterial;
            volume.GenerateBasic(FillMode.None);
            volume.MeshingMode = meshingMode;

            using (BinaryReader stream = new BinaryReader(new FileStream(fn, FileMode.Open)))
            {
                FromMagica(stream, newObject, voxelSize, centerPivot, lodScale);
            }
        ...
        public static void MagicaVoxelImport(Volume existingVolume)
        {
            ...
            {
                FromMagica(stream, existingVolume.gameObject, existingVolume.VoxelSize, false, 0);
            }
        }
