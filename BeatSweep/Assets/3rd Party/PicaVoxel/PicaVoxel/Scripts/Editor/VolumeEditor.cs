                ...
                for (float d = 0f; d < voxelObject.VoxelSize*200f; d += voxelObject.VoxelSize*0.1f)
                {
                         ...
                         (EditorPersistence.CursorMode == EditorCursorMode.BrushAdd && !EditorPersistence.BrushReplace)))
                    {
                            ...
                            EditorPersistence.CursorMode != EditorCursorMode.BrushSubtract)
                        {
                            ...
                            Handles.CubeHandleCap(0, cursorPosition, voxelObject.transform.rotation, voxelObject.VoxelSize, e.type);
                            Repaint();
                             ...
                             EditorPersistence.CursorMode != EditorCursorMode.BrushSubtract))
                        {
                            ...
                            if (propogateAllFrames)
                            {
                                ...
                                foreach (Frame f in voxelObject.Frames)
                                {
                                    f.EditingVoxels = new Voxel[voxelObject.XSize*voxelObject.YSize*voxelObject.ZSize];
                            ...
                            EditorPersistence.SelectBox.BottomLeftFront.Y)
                    {
                                ...
                                EditorPersistence.SelectBox.BottomLeftFront.Z)
                        {
                            ...
                            newPos = EditorHandles.DragHandle(handlePos, HandleUtility.GetHandleSize(handlePos)*0.25f,
                                Handles.SphereHandleCap, Color.red, out dhResult);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!runtimeOnlyMesh.boolValue)
            {
                if (serializedObject.targetObjects.Length < 2)
                {
                    ...
                    if (voxelObject.IsEnabledForEditing)
                    {
                        propogateAllFrames = EditorGUILayout.ToggleLeft(" Propagate edits to all frames", propogateAllFrames);
            ...
            EditorGUILayout.LabelField("Size (Volume size: " + voxelObject.XSize+","+voxelObject.YSize+","+voxelObject.ZSize+" Chunk Size: " + voxelObject.XChunkSize + "," + voxelObject.YChunkSize + "," + voxelObject.ZChunkSize + ")", new GUIStyle() {fontStyle = FontStyle.Bold});
            ...
            if (serializedObject.targetObjects.Length < 2 && !voxelObject.IsEnabledForEditing)
            {
                if (GUILayout.Button("Resize"))
                {
                    EditorResizeWindow window =
                        (EditorResizeWindow)EditorWindow.GetWindowWithRect((typeof(EditorResizeWindow)), new Rect(100, 100, 400, 240), true);
            ...
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Scroll Voxels", new GUIStyle() { fontStyle = FontStyle.Bold });
            ...
            EditorGUILayout.LabelField("Mesh Compression", new GUIStyle() { fontStyle = FontStyle.Bold });
            meshCompression.enumValueIndex =
                Convert.ToInt16(EditorGUILayout.EnumPopup((ModelImporterMeshCompression)meshCompression.enumValueIndex));
			...
			physicMaterial.objectReferenceValue = EditorGUILayout.ObjectField("Physic Material: ", physicMaterial.objectReferenceValue, typeof (PhysicMaterial),
			                                                            false);
            ...
            if (voxelObject.ImportedFrom != Importer.None && !string.IsNullOrEmpty(voxelObject.ImportedFile) && serializedObject.targetObjects.Length < 2 && !voxelObject.IsEnabledForEditing)
            {
                EditorGUILayout.Space();
                if (GUILayout.Button(new GUIContent(voxelObject.ImportedFrom==Importer.Magica?"Re-import from MagicaVoxel":"Re-import from Image", voxelObject.ImportedFrom == Importer.Magica ? "Re-import from original .VOX file" : "Re-import from original image file")))
                {
