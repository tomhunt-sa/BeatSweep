...
namespace PicaVoxel
{
    ...
    public class Volume : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        private void Awake()
        {
        	// NDH - Adding check here as we don't always care about the Hitbox
...
//            transform.FindChild("Hitbox").transform.localPosition = -Pivot;
            ...
            var chunks = frame.transform.Find("Chunks");
            
			for (int i = 0; i < chunks.childCount; i++)
			{
				Renderer renderer = chunks.GetChild(i).GetComponent<Renderer>();
            ...
            if (chunks.childCount == 0)
            {
                min = max = Vector3.zero;
            }
			
			Hitbox.size = max - min;
			Hitbox.center = Vector3.Lerp(min, max, 0.5f);
			Hitbox.transform.localPosition = Vector3.zero;
			
			Hitbox.material = PhysicMaterial;
