    ...
    public class Frame : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public void UpdateAllChunks()
        {
           // Debug.Log("UpdateAllChunks " + ParentVolume.transform.name);
			...
			int ceilXSize = (int) Mathf.Ceil((float) XSize/ParentVolume.XChunkSize);
			int ceilYSize = (int) Mathf.Ceil((float) YSize/ParentVolume.YChunkSize);
			int ceilZSize = (int) Mathf.Ceil((float) ZSize/ParentVolume.ZChunkSize);

			for (int x = 0; x < ceilXSize; x++)
            {
				for (int y = 0; y < ceilYSize; y++)
                {
					for (int z = 0; z < ceilZSize; z++)
                    {
#if UNITY_EDITOR
	                ...
	                if (!Application.isPlaying)
	                {
						float t = (float)(progress) / (float)(ceilXSize * ceilYSize * ceilZSize);
						UnityEditor.EditorUtility.DisplayProgressBar("PicaVoxel - " + ParentVolume.name, "Updating Chunks", t);
                	}
#endif
				}
            }
        ...
        public void CreateChunks()
        {
			...
			// NDH - Disabling saving chunk meshes because I combine and serialize them myself
